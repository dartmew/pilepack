...
from codepile.cli import main, _generate_report
import sys
...
def test_cli_basic(test_project, capsys, monkeypatch):
    monkeypatch.setattr(sys, "argv", ["codepile", str(test_project), "--no-content"])
    main()
...
def test_cli_output_file(test_project, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["codepile", str(test_project), "-o", str(out_file)])
    main()
...
def test_cli_invalid_dir(capsys, monkeypatch):
    monkeypatch.setattr(sys, "argv", ["codepile", "/nonexistent"])
